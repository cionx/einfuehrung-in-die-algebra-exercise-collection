...
\begin{solution}
  \begin{enumerate}
    ...
    \item
      ...
      Sind dann $g, h \in \powerseries{\Integer}{X}$ mit $g = \sum_{i=0}^\infty g_i X^i$, $h = \sum_{i=0}^\infty h_i X^i$ und $f = gh$, so gilt inbesondere $2 = g_0 h_0$.
    ...
    \item
      Die Aussage ist falsch:
      Die Abbildung $\sigma$ ist zwar ein Ringhomomorphismus (siehe Übung~\ref{question: the frobenius homomorphism}), aber betrachtet man etwa den Fall $R = \Field_p[X]/(X^p)$, so gilt
      \[
          \sigma(\overline{X})
        = \overline{X}^p
        = \overline{X^p}
...
\begin{question}[subtitle = Irreduziblität von Polynomen]
...
%     \item
%       Reduzieren bezüglich des Primelements $3 \in \Integer$ liefert $\tilde{f}(X) = X^3 - 4X - 1 = X^3 + 2X + 2 \in \Field_3[X]$.
...
\begin{solution}
  ...
  Wir zeigen, dass die Isomorphieklassen der gegebenen Ringe durch
  \[
    \{R_1\},
    \{R_2, R_5\},
    \{R_3, R_6\},
...
\begin{question}[subtitle = Charakterisierung von Primidealen]
...
\begin{question}[subtitle = Ideale in Quotienten]
...
\begin{question}[subtitle = Noethersch und Hauptidealring für Quotienten]
...
\begin{question}[subtitle = Radikale]
...
\begin{question}[subtitle = Gegenbeispiele]
...
\begin{question}[subtitle = Polynomringe als Hauptidealringe]
...
\begin{question}[subtitle = Die Einheitengruppe des Potenzreihenrings]
  ...
  Zeigen Sie, dass
  \[
      \powerseries{R}{T}^\times
    = \left\{
        \sum_{i=0}^\infty f_i T^i \in \powerseries{R}{T}
      \,\middle|\,
        f_0 \in R^\times
      \right\} \,.
  \]
...
\begin{solution}
  ...
  Da $R$ faktoriell ist, gibt es dann ein Primelement $p \in R$, dass alle Koeffizienten von $fg$ teilt.
...
\begin{question}[subtitle = Koeffizientenideale in Polynomringen]
  ...
  \end{enumerate}
  Damit erhalten wir für jedes Ideal $\aideal \subseteq R$ einen Ringisomorphismus
  \[
            R[X]/(\aideal)
    \to     (R/\aideal)[X] \,,
    \quad   \overline{\sum_i f_i X^i}
    \mapsto \sum_i \overline{f_i} X^i \,.
  \]
  
  \begin{enumerate}[resume]
    \item
      Veinfachen Sie für die folgenden Ringe $R$ und Ideale $I \subseteq R$ jeweils den Quotienten $R/I$.
      Entscheiden Sie jeweils anschließend, ob das Ideal prim oder maximal ist.
      \[
...
\begin{question}[subtitle = Produkte von Idealen]
...
\begin{question}[subtitle = Lokalisierung und Produkte]
  ...
  \begin{enumerate}[resume]
    \item
      Zeigen Sie, dass es eine eindeutigen Ringhomomorphismus
      \[
                \varphi
        \colon  (R \times R')_{S \times S'}
        \to     R_S \times R'_{S'}
      \]
      gibt, so dass das folgende Diagram kommutiert:
      \[
...
\begin{question}[subtitle = Funktorialität von Lokalisierung]
...
\begin{question}[subtitle = Lokalisierung durch Quotienten]
  Es sei $R$ ein kommutativer Ring und $f \in R$.
  Zeigen Sie, dass $R_f \cong R[X]/(fX-1)$ gilt.
  (Dabei ist $R_f = S_f^{-1} R$ für die multiplikative Menge $S_f = \{1, f, f^2, \dotsc\} = \{f^n \mid n \in \Natural\}$.)
\end{question}
...
\begin{solution}
  ...
  \[
    ...
    = 0.
  \]
  Also faktorisiert $\tilde{\psi}$ über einen Ringhomomorphismus $\psi \colon R[X]/(fX-1) \to R_f$ mit $\psi(\overline{p}) = \tilde{\psi}(p)$ für alle $p \in R[X]$, d.h.\ es gilt
  \[
