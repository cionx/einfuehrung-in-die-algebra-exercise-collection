...
\begin{question}[subtitle = Der Frobeniushomomorphismus]
  ...
  \begin{enumerate}
    ...
    \item
      Zeigen Sie, dass $\sigma$ ein Automorphismus ist, falls $R$ endlich ist.
  \end{enumerate}
\end{question}
...
\begin{question}
  Es sei $R$ ein Hauptidealring und $(a_i)_{i \in I}$ eine Familie von Elementen $a_i \in R$.
  \begin{enumerate}
    \item
      Es sei $a$ ein größter gemeinsamer Teiler der $a_i$.
      Zeigen Sie, dass $(a_i \mid i \in I) = (a)$.
    \item
      Entscheiden Sie, ob die obige Gleicheit auch dann noch gilt, wenn $R$ nur ein faktorieller Ring ist.
  \end{enumerate}
\end{question}
...
\begin{remark}
  Übung~\ref{question: product of ideals is an ideal} und Übung~\ref{questions: ideals in products are products of ideal} ergeben zusammen eine Klassifikation der Primideale in $R_1 \times \dotsb \times R_n$:
  Es handelt sich um die disjunkte Vereinigung der Primideale der $R_i$.
\end{remark}
...
\begin{solution}
  \begin{enumerate}
    ...
    \item
      Die Aussage ist wahr, denn jeder endliche Integritätsbereich ist bereits ein Körper.
  \end{enumerate}
\end{solution}
