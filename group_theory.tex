...
\begin{question}[subtitle = Zur Ordnung]
  ...
  \begin{enumerate}
    ...
    \item
      Entscheiden Sie, ob die vorherige Aussage auch dann noch gilt, wenn $N$ nicht normal in $G$ ist.
  \end{enumerate}
\end{question}
...
\begin{solution}
  \begin{enumerate}
    ...
    \item
      Ist $N$ nicht normal in $G$, so kann es auch noch andere Untergruppen von Ordnung $|N|$ geben.
      Man betrachte etwa die symmetrische Gruppe $G = S_3$ und die Untergruppen $H_1 = \generate{(1\;2)}$, $H_2 = \generate{(1\;3)}$, $H_3 = \generate{(2\;3)}$ mit $|H_i| = 2$ und $[G : H_i] = 3$.
  \end{enumerate}
\end{solution}
...
\begin{question}[subtitle = Untergruppen von Index $2$]
  ...
  \begin{enumerate}
    \item
      Es sei $N \subseteq G$ eine Untergruppe vom Index $[G : N] = 2$.
...
\begin{question}[subtitle = Charakteristische Untergruppen]
  ...
  \begin{enumerate}
    ...
    \item
      Es sei $N \subseteq G$ eine normale Untergruppe und $K \subseteq N$ eine charakteristische Untergruppe.
      Zeigen Sie, dass die Untergruppe $K$ bereits normal in $G$ ist.
    \item
...
\begin{solution}
  \begin{enumerate}
    ...
    \item
      ...
      Da $N$ normal ist, schränkt sich der Automorphismus $c_g$ zu einem Automorphismus $c_g|_N \colon N \to N$ ein (das Inverse ist durch die Einschränkung der Konjugation mit $g^{-1}$ gegeben).
      Da $K$ charakteristisch in $N$ ist, gilt $g K g^{-1} = c_g(K) = c_g|_N(K) = K$.
    \item
    ...
    \item
      ...
      \[
        ...
        =         \left\{
                    ...
                    \end{pmatrix}
                  \right\}
        \subseteq \GL_p(\Field_p)
      \]
      ist nicht abelsch.
      Es gilt aber $A^p = I$ für jede Matrix $A \in G$, denn das charakteristische Polynom von $A$ ist $\chi_A(t) = (t-1)^p = t^p - 1$, und nach dem Satz von Cayley--Hamilton gilt deshalb $0 = \chi_A(A) = A^p - I$.
  \end{enumerate}
\end{solution}
...
\begin{question}[subtitle = Vereinigung von Konjugaten]
...
\begin{question}[subtitle = Ein wenig Sylow]
...
%       Zeigen Sie, dass eine $p$-Sylow\-un\-ter\-grup\-pe $H$ von $G$ genau dann normal ist, wenn sie die einzige $p$-Sylow\-unter\-gruppe von $G$ ist.
%     \item
...
%   \begin{enumerate}
%     \item
%       Für jedes $g \in G$ ist auch $g H g^{-1}$ eine $p$-Sylow\-unter\-gruppe von $G$.
%       Ist $H$ die einzige $p$-Sylow\-unter\-gruppe von $G$, so ist deshalb $gHg^{-1} = H$ für alle $g \in G$, also $H$ normal in $G$.
%       
%       Ist andererseits $H$ normal in $G$ und $H'$ eine weitere $p$-Sylow\-unter\-gruppe von $G$, so gibt es nach den Sylowsätzen ein $g \in G$ mit $H' = g H g^{-1}$.
