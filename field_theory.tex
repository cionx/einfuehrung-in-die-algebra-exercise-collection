...
\begin{solution}
  \begin{enumerate}
    ...
    \item
      Die Aussage ist wahr:
      Sie wurde auf einem der Übungszettel gezeigt, wir geben hier der Vollständigkeit halber dennoch einen Beweis an:
      
      Wir zeigen die Aussage per Induktion über $n$.
      Für $n = 1$ gilt $L = K$ und die Aussage gilt.
      Es sei nun $n \geq 2$, und die Aussage gelte für alle Polynome vom Grad $< n$.
      
      Wir betrachten zunächst den Fall, dass $f(t)$ irreduzibel ist.
      Dann sei $a \in L$ eine Nullstelle von $f(t)$.
      Dann ist $f(t)$ das Minimalpolynom von $a$ über $K$ und deshalb $[K(a) : K] = n$.
      Über $K(a)$ lässt sich von $f(t)$ der Linearfaktor $(t-a)$ abspalten, d.h.\ es gibt ein Polynom $g(t) \in K(a)[t]$ mit $f(t) = g(t) (t-a)$.
      Dabei gilt insbesondere $\deg g(t) = \deg f(t) - 1 = n - 1$.
      Dann ist $L$ auch von $g(t)$ ein Zerfällungskörper, und nach Induktionsvoraussetzung gilt deshalb $[L : K(a)] \mid (n-1)!$.
      Zusammen mit $[K(a) : K] = n$ erhalten wir, dass
      \[
              [L : K]
        =     [L : K(a)] [K(a) : K]
        \mid  (n-1)! \, n
      ...
      Allgemeiner sei $g(t)$ ein irreduzibler Faktor von $f(t)$, und es sei $h(t) \in K[t]$ mit $f(t) = g(t)h(t)$.
      Sind dabei $d_1 \coloneqq \deg g(t)$ und $d_2 \coloneqq \deg h(t)$, so gilt $n = d_1 + d_2$.
      Es enthält $L$ einen Zerfällungskörper $L'$ von $g(t)$.
      Fassen wir $h(t)$ als ein Polynom über $L'$ auf, also als $h(t) \in L'[t]$, so ist $L$ ein Zerfällungskörper von $h(t)$ über $L'$.
      Nach Induktionsvoraussetzung gelten nun
      \begin{gather*}
              [L' : K]
        \mid  d_1!
        \quad\text{and}\quad
...
\begin{question}[subtitle = Rechnen in zyklischen Erweiterungen]
...
\begin{question}[subtitle = Die Erweiterung $\Rational(\sqrt{2} + \sqrt{3})/\Rational$]
...
\begin{question}[subtitle = {Galoisgruppe von $\Rational(\sqrt[3]{3}, \zeta)/\Rational$}]
...
\begin{question}[subtitle = {Galoisgruppe von $\Rational(\sqrt[4]{2}, i)/\Rational$}]
...
\begin{question}[subtitle = Galoisgruppe von $\Real/\Rational$]
...
\begin{question}[subtitle = Galoisgruppe von $X^p - 2$]
...
\begin{question}[subtitle = Unendlichkeit algebraisch abgeschlossener Körper]
...
\begin{question}[subtitle = Algebraischen Abschlüsse von Unterkörpern]
...
\begin{question}[subtitle = You should be able to do this]
...
\begin{question}[subtitle = {Algebraizität von $\alpha$ und $\beta$}]
  Es sei $L/K$ eine Körpererweiterung und es seien $\alpha, \beta \in L$.
  Zeigen Sie, dass $\alpha$ und $\beta$ genau dann beide algebraisch über $K$ sind, wenn $\alpha + \beta$ und $\alpha \beta$ beide algebraisch über $K$ sind.
\end{question}
...
\begin{remark*}
  Da $\pi$ und $e$ transzenent (über $\Rational$) sind, muss $\pi + e$ oder $\pi e$ transzendent sein.
...
\begin{solution}
  Sind $\alpha$ und $\beta$ algebraisch über $K$, so ist $K(\alpha, \beta) / K$ eine algebraische Körpererweiterung.
  Da $\alpha + \beta, \alpha \beta \in K(\alpha, \beta)$ gilt, sind $\alpha + \beta$ und $\alpha \beta$ dann algebraisch über $K$.
...
\begin{question}[subtitle = Zur Separablität]
...
\begin{question}[subtitle = Charakterisierung algebraischer Erweiterungen durch Zwischenringe]
